<#PSScriptInfo
.VERSION        1.0.5
.AUTHOR         @MrTbone_se (Original), Modified by Andrew Blunt
.GUID           feedbeef-beef-4dad-beef-85924e86a608
.COPYRIGHT      (c) 2026 T-bone Granheden. Modifications by Andrew Blunt. MIT License.
.TAGS           Intune Graph PrimaryUser DeviceManagement MicrosoftGraph Azure SecureBoot
...
.RELEASENOTES
    ...
    1.0.5 2026-01-23 Integrated KEK hardware capability check (Custom Mod)
#>
...
.DESCRIPTION
    ...
    MODIFIED: Added KEK (Key Exchange Key) check to identify if hardware supports the update.

.NOTES
    Original Author: @MrTbone_se
    Modified by: Andrew Blunt (2026-01-23)
...
.NOTES
    Version: 1.0.2
    Author:  @MrTbone_se (Original), Modified for boolean return.
#>
    try {
        ...
        if ([System.Text.Encoding]::ASCII.GetString($db.Bytes) -match "Windows UEFI CA 2023") {
            ...
            Write-Warning "[INFO]CA 2023 not in UEFI DB yet" 
            return $false
        }
    }
    catch { 
        Write-Warning "[INFO]Unable to read UEFI DB: $_" 
        return $null
    }
}
function Detect-SecureBootKEK2023 {
    <#
.SYNOPSIS
    Check if Secure Boot CA 2023 certificate is present in UEFI KEK (hardware support)
.NOTES
    Version: 1.0.0
    Author:  Andrew Blunt (Added 2026-01-23)
#>
    try {
        $kek = Get-SecureBootUEFI -Name kek -EA Stop
        if ([System.Text.Encoding]::ASCII.GetString($kek.Bytes) -match "2023") {
            Write-Host "[OK]CA 2023 KEK found in UEFI (Hardware Capable)"
            return $true
        }
        else { 
            Write-Warning "[ERR]CA 2023 KEK not in UEFI. Hardware/BIOS update required before OS update."
            return $false
        }
    }
    catch { 
        Write-Warning "[INFO]Unable to read UEFI KEK: $_" 
        return $null
    }
}
            ...
            switch ($statusValue) {
                ...
                default {
                    ...
                    elseif ($RemediateMode) {
                        ...
                        # Hardware check (KEK) vs Servicing check (DB)
                        $kekReady = Detect-SecureBootKEK2023
                        $dbReady = Detect-SecureBootCA2023
                        Detect-TPMEvent1808
                        
                        if ($kekReady -eq $false -and $dbReady -eq $false) {
                            Write-Error "CRITICAL: Device is BLOCKED / AT RISK. BIOS update required to enable 2023 KEK support."
                        }
